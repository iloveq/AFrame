...
public abstract class BaseFragment extends Fragment implements NetworkStateView.OnRetryClickListener {
    ...
    private void initDialog() {
        progressDialog = new ProgressDialogUtils(getActivity());
    }
     ...
     * 显示没有网络的布局
     */
    public void showNoNetworkRetryView() {
        networkStateView.showNoNetworkRetry();
     ...
     * @param time    需要显示的时间长度(以毫秒为单位)
     */
    public void showProgressDialogFail(String message, long time) {
        if (isAdded())
     ...
     * @param message 加载成功需要显示的文字
     */
    public void showProgressDialogFail(String message) {
        if (isAdded())
